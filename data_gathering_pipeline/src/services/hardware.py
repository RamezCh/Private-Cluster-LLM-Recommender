...
def is_moe_model(model_name: str, config_tags: Optional[List[str]] = None) -> Tuple[bool, Optional[int]]:
    """Detect if a model is Mixture-of-Experts."""
    model_lower = model_name.lower()
    
    moe_indicators = [
        "moe", "mixture", "deepseek-v3", "deepseek-v4",
        "qwen-moe", "mixtral", "mixtral-8x7b", "dbrx", "switch-transformer"
    ]
...
def estimate_size_from_params(param_count: Optional[int] = None, size_bytes: Optional[int] = None) -> float:
...
def get_recommended_context_tier(intelligence_index: Optional[float] = None, model_name: Optional[str] = None) -> str:
    ...
    for gpu_id, gpu in GPU_CATALOG.items():
        for count in [1, 2, 4, 8]:
            ...
            if is_moe:
                ...
                strategy = "Single-GPU"
                parallelism = "Single-GPU" if count == 1 else f"Data Parallelism (DP={count})"
            else:
            ...
            results.append(MultiHardwareFit(
                gpu_id=gpu_id,
                gpu_name=gpu.name,
                vram_gb=gpu.vram_gb,
                count=count,
...
def format_all_fits(vram_req: VRAMRequirements, is_moe: bool = False) -> Dict:
    ...
    by_tier = {tier: [] for tier in ["data_center", "professional", "consumer", "laptop", "legacy"]}
    ...
    return {
        "all_compatible_gpus": [
            {"name": f.gpu_name, "count": f.count, "vram": f.total_vram, "score": f.compatibility_score}
            for f in compatible[:10]
        ],
        "by_tier": by_tier,
        "best_data_center": by_tier["data_center"][0] if by_tier["data_center"] else None,
