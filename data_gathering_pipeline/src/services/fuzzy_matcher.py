...
class FuzzyModelMatcher:
    ...
    def _select_canonical(self, names: List[str]) -> str:
        ...
        # Patterns with explicit priority weights (higher = better canonical)
        patterns = [
            (r"\d+\.\d+", 30),   # has version number (most specific)
            (r"[A-Z][a-z]", 20),  # proper casing
            (r"instruct", 10),    # instruction-tuned variant
            (r"chat", 5),         # chat variant
        ]
        best, best_score = names[0], 0

        for name in names:
            score = 50 + sum(
                weight if re.search(pat, name, re.I) else 0
    ...
    def build_mappings(
        ...
        logger.info(
            f"Building mappings: {len(source_a)} + {len(source_b)} + {len(source_c or [])}"
        )
        ...
        for base, names in groups.items():
            ...
            if not self.mappings[canonical].is_confident and len(names) > 1:
                self.unmatched.append(
                    {"canonical": canonical, "variants": names, "score": score}
                )

        logger.info(
            f"Created {len(self.mappings)} mappings, {len(self.unmatched)} low-confidence"
        )
