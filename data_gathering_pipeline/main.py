...
def run_merge_only(args):
    ...
    orchestrator = Orchestrator(output_path=args.output or OUTPUT_FILE)
    orchestrator.raw_data["artificial_analysis"] = json.loads(TEMP_PERFORMANCE_FILE.read_text())
    ...
    all_names = (
        [item.get("model_name") for item in orchestrator.raw_data["artificial_analysis"]] +
    ...
    orchestrator.matcher.build_mappings(
        [item.get("model_name") for item in orchestrator.raw_data["artificial_analysis"] if item.get("model_name")],
        dataset_loader.get_model_names("open_evals"),
        dataset_loader.get_model_names("lmsys_arena")
    )
    ...
    for model_name in unique_names:
        try:
            from src.services import (
                FuzzyModelMatcher, HFMetadataService, VRAMCalculator,
                is_moe_model, parse_model_size, estimate_size_from_params,
                get_recommended_context_tier, format_all_fits
            )
            
            canonical = orchestrator.matcher.get_canonical(model_name) or model_name
            
            aa_item = next((i for i in orchestrator.raw_data["artificial_analysis"] 
                          if i.get("model_name") == canonical), None)
            oe_item = next((i for i in orchestrator.raw_data["open_evals"] 
                          if i.get("model_name") == canonical or i.get("name") == canonical), None)
            lmsys_item = next((i for i in orchestrator.raw_data["lmsys_arena"]
                              if i.get("name") == canonical or i.get("title") == canonical), None)
            
            from src.orchestrator import BenchmarkMerger
            benchmarks = BenchmarkMerger.merge(aa_item, oe_item, lmsys_item)
            context_tier = get_recommended_context_tier(benchmarks.get("intelligence_index"), canonical)
...
def run_report(args):
    ...
    strategies = {"Single-GPU": 0, "TP-Sharded": 0, "Expert-Distributed": 0}
    moe_count = 0
    avg_elo = []
...
Examples:
  ...
  python main.py --report       Generate report from data
    ...
    parser.add_argument("--scrape-only", action="store_true", help="Run only web scraper")
    parser.add_argument("--merge-only", action="store_true", help="Merge using cached data")
    parser.add_argument("--report", action="store_true", help="Generate report from data")
    parser.add_argument("--output", type=Path, default=OUTPUT_FILE, help="Output file path")
    parser.add_argument("--input", type=Path, default=OUTPUT_FILE, help="Input file for --report")
    parser.add_argument("--visible", action="store_true", help="Show browser during scraping")
